...
func main() {
	...
	if existing.Error != nil {
		...
		if existing.Error == gorm.ErrRecordNotFound {
			...
			log.Fatal(existing.Error)
		}
	} else {
		if err := database.DB.Exec("UPDATE profiles SET password = ?, display_name = ?, email = ? WHERE LOWER(email) = ?", string(hashedPassword), "Admin Mas", email, email).Error; err != nil {
			log.Fatal(err)
		}
	}

	var profile2 models.Profile
	if err := database.DB.Where("LOWER(email) = ?", email).First(&profile2).Error; err != nil {
		log.Fatal(err)
	}
